...
fn test_seqads() {
    ...
    let mut bz = [0; 300];
    let size = ads_wrap.read_entry(2, &key_hash, &address, &mut bz);
    assert!(size != 0);
    let entry_bz = EntryBz { bz: &bz[..size] };
    assert_eq!(entry_bz.value(), &[2; 20]);
    let mut bz = [0; 300];
    let size  = ads_wrap.read_entry(2, &key_hash2, &address2, &mut bz);
    assert!(size != 0);
    let entry_bz = EntryBz { bz: &bz[..size] };
    assert_eq!(entry_bz.value(), &[4; 20]);
    let mut bz = [0; 300];
    let size = ads_wrap.read_entry(2, &key_hash3, &address3, &mut bz);
    assert!(size != 0);
    let entry_bz = EntryBz { bz: &bz[..size] };
